...
def load_words():
    ...
    inFile = open("d:\\Coding\\Python-Practice\\hangman\\words.txt",
                  'r')  # Unfortunately inFile = open('words.txt', 'r') doesn't work at all. Please change directory
    # before use!!
    line = inFile.readline()
    list1 = line.split()
    print("  ", len(list1), "words loaded.\n")
    return list1
...
def get_guessed_word(guessed, missed_letters):
    """
    ...
    while True:
        ...
        guess = input()
        guess.lower()
        if (guess in guessed) or (guess in missed_letters):
...
def get_guessed_word_hints(guessed, missed_letters):
    """
    secret_word: string, the word the user is guessing
    letters_guessed: list (of letters), which letters have been guessed so far
    missedLetters: list (of letters), which have not benn guessed so far
...
def avaliable(missed_letters, guessed):
    missed_letters = set(missed_letters)
    alphabet = set("abcdefghijklmnopqrstuvwxyz")
    letters = alphabet.difference(missed_letters)
    letters = letters.difference(guessed)

    return letters


def get_available_letters(missed_letters, input_word):
    """
    letters_guessed: list (of letters), which letters have been guessed so far
...
def hangman(secret):
    ...
    * After each guess, you should display to the user the
    ...
    if (attempts > 0) and (gameIsDone == True):
        ...
        missedLetters, gameround_result, is_vowel = get_available_letters(missedLetters, guess)
